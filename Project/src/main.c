...
int main(void)
{
  ...
  /* Initialize TAMPER Button mounted on STM324xG-EVAL board */       
  //STM_EVAL_PBInit(Button_TAMPER, Mode_GPIO);  
  STM_EVAL_PBInit(Button_WAKEUP, Mode_GPIO);  
  
  
  /* Test if TAMPER push-button on STM324xG-EVAL Board is not pressed */
  ...
  { /* Key push-button not pressed: jump to user application */
    ...
    {/* Otherwise, do nothing */
      /* LED1 (RED) ON to indicate bad software (when not valid stack address) */
